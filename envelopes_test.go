...
func TestEventEnvelopeEncodingAndDecoding(t *testing.T) {
	...
	for _, raw := range eventEnvelopes {
		...
		require.Equal(t, env.Events[0].GetID(), env.Events[0].ID)

		ok, _ := env.Events[0].CheckSignature()
		require.True(t, ok)

		asJSON, err := json.Marshal(env)
		require.NoError(t, err)
		require.Equal(t, raw, string(asJSON))
	}
}
...
func TestNoticeEnvelopeEncodingAndDecoding(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "kjasbdlasvdluiasvd\"kjasbdksab\\d", string(env))

	res, err := json.Marshal(env)
	require.NoError(t, err)
	require.Equal(t, noticeEnv, string(res))
}
...
func TestEoseEnvelopeEncodingAndDecoding(t *testing.T) {
	...
	res, err := json.Marshal(env)
	require.NoError(t, err)
	require.Equal(t, eoseEnv, string(res))
}
...
func TestCountEnvelopeEncodingAndDecoding(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, int64(12), *env.Count)

	res, err := json.Marshal(env)
	require.NoError(t, err)
	require.Equal(t, countEnv, string(res))
}
...
func TestParseMessage(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			ExpectedEnvelope: &ReqEnvelope{SubscriptionID: "million", Filters: Filters{{Kinds: []int{1}}, {Kinds: []int{30023}, Tags: TagMap{}.SetLiterals("d", "buteko", "batuke")}}},
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			if testCase.ExpectedEnvelope == nil {
				require.NotNil(t, envelope, "expected nil but got %v\n", envelope)
			}
